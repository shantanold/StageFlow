...
/** Spotify-style 2×2 collage of the first item photos in a set. */
export function SetPhotoCollage({
  urls = [],
  size = 56,
}: {
  urls?: string[];
  ...
  if (photos.length === 1) {
    const src = displayPhotoUrl(photos[0]);
    return (
      <div
        style={{
          width: size,
  ...
  return (
    ...
    >
      {cells.map((url, i) => {
        const src = displayPhotoUrl(url);
        return (
          <div key={i} style={{ background: "var(--bg-surface)", overflow: "hidden", minWidth: 0, minHeight: 0 }}>
            {src ? (
              <img src={src} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
            ) : null}
          </div>
  ...
  return (
    <div className="card" style={{ cursor: "pointer" }} onClick={onClick}>
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
        <SetPhotoCollage urls={set.preview_photo_urls} size={56} />
    ...
    <>
      {[...Array(3)].map((_, i) => (
        <div
          key={i}
          className="card"
          style={{ pointerEvents: "none", display: "flex", gap: 12 }}
        >
          <div style={{ width: 56, height: 56, borderRadius: 8, background: "var(--bg-surface)", flexShrink: 0 }} />
          <div style={{ flex: 1 }}>
